...
export class ListComponent implements OnInit {
  // url = [];
  private readonly _perPage = 20;
  public pages: string[][] = []; // only the urls currently visible
  ...
  private sortUrlsIntoPages(): void {
    this.numOfPages = Math.round(this.urls.length / 20);
    let tempArr = [];

    for (let i = 0; i < this.urls.length; i++) {
      tempArr.push(this.urls[i]); // add item to temp array

      /*
        first item => i = 0 tempArr = [urls[0]]
        second item => i = 1 = [urls[0], urls[1]]
        2 / 20 = 0.1

        twentieth item:
        i = 19

        tempArr = [urls[0], urls[1],....urls[19]]
        i + 1 = 20; 20 / 20
      */
      if (
        (i !== 0 && (i + 1) % this._perPage === 0) ||
        i === this.urls.length
      ) {
        this.pages.push(tempArr);
        tempArr = [];
      }
    }
  }
